...
- **Realtime:** backend có `DuoWsManager` quản lý kết nối WebSocket theo phòng và phát trạng thái (`duo_created`, `snapshot`, heartbeat `ping`/`pong`). Cần nói thẳng ở đây: frontend hiện cập nhật trạng thái bằng cách gọi lại `getDuoSession` (polling REST) chứ chưa nối WebSocket phía client, nên đây là một điểm còn có thể nâng cấp.
...
`src/pipeline/tarot_pipeline.py` đóng vai bộ điều phối, gọi 5 module con theo trình tự ASR, cảm xúc, Vision hoặc Random, RAG, rồi LLM. Bản thân nó không chứa AI; thay vì ném lỗi, nó gom các cảnh báo (`warnings`) lại, nhờ vậy một tầng hỏng không kéo sập cả phiên đọc.
...
- **Rate limiter:** dùng sliding window in-memory (lưu mốc `time.monotonic()` trong `deque`), có `threading.Lock`; key tính theo `"{scope}:{ip}"`, với IP ưu tiên lấy từ `x-forwarded-for` ở vị trí proxy tin cậy. Vượt ngưỡng thì trả 429, và toàn bộ bật tắt qua `RATE_LIMIT_ENABLED`.
  ...
  - Nói cho rõ: các endpoint còn lại như sessions, daily, community, duo thì không gắn rate limit. Và vì là in-memory nên cách này chỉ đúng khi chạy một process; muốn multi-worker thì cần Redis.
...
- **Mẹo 1, nhận biết lá ngược:** hệ thống embed cả ảnh gốc và ảnh xoay 180 độ; ứng viên đến từ bản xoay sẽ bị đảo orientation từ xuôi sang ngược và ngược lại. Nếu bản xoay khớp tốt hơn thì nghĩa là lá đang đặt ngược, nhờ vậy không cần ảnh huấn luyện riêng cho lá ngược.
