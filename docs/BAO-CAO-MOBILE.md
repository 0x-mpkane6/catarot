...
7. [Đóng gói ứng dụng Android (PWA → TWA)](#7-đóng-gói-ứng-dụng-android-pwa--twa)
...
| React Native / Flutter + WebView | Khung native bao ngoài một WebView | Nặng (vài chục MB), vẫn mang nhược điểm WebView, chỉ đáng làm khi cần màn hình native thật |
...
```mermaid
flowchart LR
    Dev["Mã nguồn frontend<br/>(React + Vite)"] -->|npm run build| Dist["dist/ (SPA tĩnh)"]
    Dist -->|wrangler deploy| CF["Cloudflare Workers<br/>https://catarot.me"]
    CF -->|hiển thị trực tiếp| Web["Trình duyệt (desktop & mobile)"]
    CF -->|TWA bọc URL| APK["App Android<br/>(Chrome engine)"]
    Bubble["Bubblewrap (PWA → TWA)"] -.->|đóng gói 1 lần| APK
    APK -.->|mỗi lần mở nạp lại| CF
```
...
- Hook `useIsMobile` theo dõi chiều rộng màn hình rồi trả về cờ `isMobile`, dùng để đổi bố cục/markup ngay trong React.
- Các khối CSS `@media` (`max-width: 768px` / `640px` / `767.98px`) lo phần kiểu hiển thị mà không cần đụng tới JavaScript.

```mermaid
flowchart TB
    V["Khung nhìn (viewport)"] --> Q{"Rộng dưới 768px?"}
    Q -->|"Có — điện thoại"| M["isMobile = true<br/>+ @media (max-width: 767.98px)"]
    Q -->|"Không — desktop"| D["isMobile = false<br/>(giữ nguyên đường đi mã desktop)"]
    M --> M1["Bố cục mobile:<br/>lưới 2 cột · ô nhập ghim đáy ·<br/>tab ngang · hiệu ứng cảm ứng"]
    D --> D1["Bố cục desktop:<br/>zoom-to-fit · quạt bài ·<br/>hiệu ứng theo con trỏ"]
```
...
| Thanh điều hướng | Một số lối vào chỉ có trên máy tính | Đưa Cài đặt và Hướng dẫn vào menu ☰ để dùng được trên điện thoại |
| Landing / Đăng nhập | Khoảng trống lớn, thiếu lời kêu gọi | Thêm nút vào trang giới thiệu; form đăng nhập dạng thẻ kính, căn giữa |
...
**Phác thảo bố cục — Trang Chủ (mobile):**

```
┌──────────────────────────────┐
│ ☰        CATAROT       [ Bạn ]│  thanh nav (glass, cố định)
├──────────────────────────────┤
│ Chào, <tên người dùng>        │  lời chào
│ Hôm nay những lá bài muốn…    │
│ ┌──────────────────────────┐  │
│ │ [ảnh]  ✦ BẮT ĐẦU         │  │  ◀ lá chính (hero)
│ │        Trải Bài          │  │
│ └──────────────────────────┘  │
│ ┌────────────┐ ┌───────────┐  │
│ │ Hằng Ngày  │ │ Tầm Nhìn  │  │  ◀ lưới 2 cột
│ └────────────┘ └───────────┘  │
│ ┌────────────┐ ┌───────────┐  │
│ │ Bài Đôi    │ │ Cộng Đồng │  │
│ └────────────┘ └───────────┘  │
│ 🐈 ← chạm để rút 1 lá          │  mèo (chỉ ở màn này)
└──────────────────────────────┘
```

**Phác thảo bố cục — một "phòng" (mobile):**

```
┌──────────────────────────────┐
│ ☰        CATAROT       [ Bạn ]│
├──────────────────────────────┤
│ [ Bảng tin ]  [ Chia sẻ ]     │  ◀ tab NGANG (thay thanh dọc)
├──────────────────────────────┤
│  ▲ nội dung cuộn nội bộ        │
│  │ …danh sách / bài viết…      │
│  ▼                            │
│ ┌──────────────────────────┐  │
│ │  ô nhập / chia sẻ…        │  │  ◀ trong tầm với, không cuộn xa
│ └──────────────────────────┘  │
└──────────────────────────────┘
```
...
sequenceDiagram
    ...
    participant G as Màn hiện tại
    participant V as Tấm phủ (CosmicVeil)
    participant N as Màn mới
    U->>G: Chạm chọn chế độ
    G->>V: playScene(onCover)
    V->>V: Hiện kín màn (~300ms)
    V->>N: onCover() đổi nội dung khi đang bị che
    V->>V: Tan dần (~640ms)
    N-->>U: Lộ ra màn mới
```

Toàn bộ hiệu ứng chỉ dùng các thuộc tính nhẹ (transform/opacity) để giữ độ mượt trên điện thoại.
...
- Các hoạt ảnh đều ưu tiên `transform`/`opacity` thay vì những thuộc tính làm dồn lại bố cục, nhờ vậy khung hình ổn định hơn và đỡ tốn pin.
- Các màn được kiểm tra để không bị tràn ngang (không xuất hiện thanh cuộn ngang ngoài ý muốn).

---

## 7. Đóng gói ứng dụng Android (PWA → TWA)
...
- Tệp `/.well-known/assetlinks.json` đặt ngay trên domain, chứa vân tay khoá ký của app. Nhờ liên kết tin cậy này, app mở web ở chế độ toàn màn hình và bỏ luôn thanh địa chỉ, nhìn như một ứng dụng thật.
...
```mermaid
flowchart TB
    subgraph APK["APK Android — me.catarot.app (~1,65 MB)"]
        TWA["Trusted Web Activity<br/>(Chrome engine · toàn màn hình)"]
    end
    subgraph WEB["Web — https://catarot.me"]
        SPA["SPA React<br/>(giao diện + tính năng)"]
        PWA["PWA: manifest + service worker + icons"]
        AL[".well-known/assetlinks.json<br/>(liên kết tin cậy)"]
    end
    TWA -->|"nạp & hiển thị"| SPA
    TWA -. "xác thực vân tay khoá ký" .-> AL
    PWA --- SPA
```
