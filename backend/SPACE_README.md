...
- **ASR** (faster-whisper) chuyển giọng nói sang text
- **LLM** sinh luận giải tiếng Việt: Gemini, OpenAI, Ollama, rồi template fallback
...
- `RESEND_API_KEY`: gửi email đặt lại mật khẩu (đăng ký tại https://resend.com, mục API Keys). Gửi qua HTTPS nên không bị chặn cổng SMTP.
...
> fallback **SMTP** (`SMTP_HOST`+`SMTP_FROM`+…). Chưa cấu hình kênh nào thì forgot-password
...
```
GET /api/health
trả về {"status":"ok","version":"0.2.0","db":"ok",...}
```
...
- Google login cần lib `google-auth` (đã có trong requirements.txt), nhớ rebuild Space sau khi cập nhật requirements.
